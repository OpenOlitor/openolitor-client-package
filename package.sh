...
do
  ...
  ( cd .tmp/${PROJECT} && npm install -g bower grunt-cli && npm install grunt && npm install --cache-min 99999 && bower install && grunt build --env=$ENVIRONMENT --buildnr=$BUILD_NUMBER )
